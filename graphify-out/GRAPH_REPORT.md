...
## Corpus Check
- 32 files · ~99,978 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 210 nodes · 394 edges · 16 communities (14 shown, 2 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 43 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `384286cc`
...
8. `Aluno` - 10 edges
9. `Python Dependencies (requirements.txt)` - 10 edges
...
Cohesion: 0.11
Nodes (26): alterar_mensalidade(), atualizar_status_pagamento(), cadastrar_pagamento(), cadastrar_plano(), detalhes_usuario(), enviar_recado(), painel_adm(), route (+18 more)

### Community 1 - "AlunoDAO"
Cohesion: 0.15
Nodes (22): remover_usuario(), cancelar_alteracao(), editar_perfil(), mascarar_email(), pagina_cadastro(), pagina_login(), pagina_perfil(), route (+14 more)
...
Cohesion: 0.12
Nodes (14): Plano, Aluno, patch, _enviar_email_brevo(), _enviar_email_resend(), Envia e-mail via API REST HTTPS da Brevo (antigo Sendinblue) - funciona no…, Envia e-mail via API REST HTTPS da Resend - funciona no Render Free., home() (+6 more)
...
- **Why does `AlunoDAO` connect `AlunoDAO` to `adm_bp.py`, `usuarioDAO.py`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `Aluno` connect `usuarioDAO.py` to `AlunoDAO`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Are the 14 inferred relationships involving `AlunoDAO` (e.g. with `alterar_mensalidade()` and `cadastrar_pagamento()`) actually correct?**
